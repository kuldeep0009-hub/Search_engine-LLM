...
# upar wala code groq pr h or niche wala
import streamlit as st
# Correct imports for Cloud
from langchain_community.utilities.arxiv import ArxivAPIWrapper
from langchain_community.utilities.wikipedia import WikipediaAPIWrapper
from langchain_community.utilities.duckduckgo_search import DuckDuckGoSearchResults
...
st.title("LangChain - Chat with Search (Free LLM)")

# Sidebar API key (optional for HF local models)
api_key = st.sidebar.text_input("Enter HuggingFace API Key (optional):", type="password")
...
if prompt := st.chat_input("Ask me something..."):
    ...
    # Free HuggingFace LLM pipeline
    hf_pipeline = pipeline(
        "text-generation",
        model="google/flan-t5-small",  # Free small model
        max_length=200
    )
