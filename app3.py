...
st.title("LangChain Chatbot with Free HuggingFace Model")
...
generator = pipeline(
    "text-generation",
    model="tiiuae/falcon-7b-instruct",  # lightweight, free model
    device=-1,  # CPU mode
    max_length=512,
    do_sample=True,
    temperature=0.7
...
if prompt := st.chat_input(placeholder="Ask me anything..."):
    ...
    # Wrap HF pipeline in a simple callable for LangChain
    class HFLLMWrapper:
        def __call__(self, input_text):
            output = generator(input_text)[0]["generated_text"]
            return output
